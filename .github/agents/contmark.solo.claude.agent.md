...
1. `Bash: ws=""; d="$(pwd)"; while [ "$d" != "/" ]; do if [ -f "$d/.asta-context/workspace.yml" ]; then ws="$d"; break; fi; d="$(dirname "$d")"; done; echo "${ws:-NONE}"`
...
7. **Per `$repo` in `$repo_order` (topo-sorted):** `workdir = (SINGLE ? $root : <$root>/<$repo>)`, `Bash: cd workdir` · `$workspace_context_dir = <$root>/.asta-context` · `$repo_context_dir = <$root>/.asta-context/repos/<$repo>` · read ONLY `$matches WHERE repo == $repo` at `source:line` · read `_pins.yml` → `$skills.*` · run Boot → Stage 6.
...
1. Resume first `[ ]` under `### Unit Test`. `Grep` `Stubs.java`/`Stubs.kt` first — reuse builders.
...
- Jira + evolution failures never block · never guess file paths — verify via `Glob`/`Grep`
