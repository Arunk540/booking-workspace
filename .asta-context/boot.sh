...
# Usage: sh .asta-context/boot.sh "<key nouns from the task>"
d="$(pwd)"
while [ "$d" != "/" ]; do
  [ -f "$d/.asta-context/workspace.yml" ] && break
  d="$(dirname "$d")"
done
if [ "$d" = "/" ]; then echo "NO_ASTA_CONTEXT"; exit 1; fi
echo "ROOT: $d"
echo "== RESOLVER =="
node "$d/.asta-context/resolve-task.js" "$d" "$*" 2>&1 | head -60
echo "== WORKSPACE LESSONS (first 40 lines) =="
sed -n '1,40p' "$d/.asta-context/lessons.md" 2>/dev/null
for r in "$d"/.asta-context/repos/*/; do
  n="$(basename "$r")"
